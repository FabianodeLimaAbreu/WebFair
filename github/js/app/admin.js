window.Admin = Spine.Controller.sub({
   elements:{
    ".table":"table", ".tab-list a":"tabs", ".tab-title":"tabtitle", "#main .text":"inputs", "#main":"clientEl", ".scrollContent":"tbody"
}, events:{
    "click .tab-list a":"tabsel", "click .enter-cart":"enter", "click .tab-close":"tabclose", "click .bclose":"close", "click .back":"close"
}, close:function(a) {
  if("object" === typeof a) {
...
window.Spotlight = Spine.Controller.sub({elements:{dd:"buttons"}, events:{"click dd":"select"}, select:function(a) {
  ...
  if(48 <= a.keyCode && 90 >= a.keyCode || 8 == a.keyCode) {
    if(!isNumber(c)){
        b = this.spot.filter(function(a) {return-1 !== a.Representante.toLowerCase().indexOf(c.toLowerCase())});
    }else{
        b = this.spot.filter(function(a) {return-1 !== a.Cod.indexOf(c)});
    }
    c = null;
    for(d in b) {
  ...
  this.input = null;
  this.doc = $(document);
  $.getJSON("http://was-dev/Focus24Dev/Services/Cliente.svc/repre/?callback=?", this.proxy(function(a) {
    this.spot = a
  }));
  this.el.disableSelection()
}});
window.Client = Spine.Controller.sub({elements:{"#spotlight":"spotEl", "#modal":"modalEl", form:"main", "form input":"inputs", ".insc":"insc", ".delete":"deleteBt", ".toprep":"repEl", ".select-cart":"isento", "#wrap .mask":"maskEl"}, events:{"submit .client-form":"submit", "click .delete":"delitem", "click .bconfirm":"cep", "blur .cep":"cep", "focus .combo-input":"prompt", "click .bprompt":"prompt", "click .combo li":"combo", "click .select-cart":"select"}, init:function() {
  ...
  d && $.getJSON("http://was-dev/Focus24Dev/Services/Cliente.svc/client/" + e + "/?callback=?", function(a) {
    -1 !== a.indexOf("salvo") && $.getJSON("http://was-dev/Focus24Dev/Services/Cliente.svc/enviaSAP/" + g + "?callback=?", function(a) {
  ...
  $.getJSON("http://was-dev/Focus24Dev/Services/Cliente.svc/delete/" + this.cnpj + "?callback=?", this.proxy(this.getout))
}, getout:function() {
  ...
  var b, d = this;
  (a = a.val() || a.prev().val()) && $.getJSON("http://was-dev/Focus24Dev/Services/CEP.svc/cep/" + a + "?callback=?", function(a) {
